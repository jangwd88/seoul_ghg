    ...
    imgObj.src = installPath + "common/SpreadJS/filter.png";
    
    function ImageButtonCellType(label) {
        GC.Spread.Sheets.CellTypes.Button.apply(this, arguments); // 추가됨
    ...
    ImageButtonCellType.prototype = new GC.Spread.Sheets.CellTypes.Button();
    ...
    function newRowFilter(dataType, containerID) {
        if (dataType == "STRING") return (new StringRowFilter(containerID));
        if (dataType == "NUMBER") return (new NumberRowFilter(containerID));
    }

// Custom필터는 Condition객체를 상속함 (evaluate를 구현)
    function StringRowFilter(containerID) { // 필터col별로 유지(binder(sheet) scope)
        ...
        // 이하는 instance멤버
        //this.binder = binder;
        this.binder = aiXtabLoader.getBinderFromID(containerID);
    ...
    StringRowFilter.prototype = new GC.Spread.Sheets.ConditionalFormatting.Condition();
    StringRowFilter.prototype.evaluate = function(evaluator, row, col) { // IF over-ride
    ...
    StringRowFilter.prototype.initDialog = function(col) { // ES 5이상 (arr.indexOf)
    ...
    StringRowFilter.prototype.onUserSelected = function() {
        this.spanned = false;
        var controls = document.forms["f-d-s-form"].elements;
    ...
    function NumberRowFilter(containerID) { // 필터col별로 유지(binder(sheet) scope)
        GC.Spread.Sheets.ConditionalFormatting.Condition.apply(this, arguments);
    ...
    NumberRowFilter.prototype = new GC.Spread.Sheets.ConditionalFormatting.Condition();
    NumberRowFilter.prototype.evaluate = function(evaluator, row, col) { // 필터 IF
        // evaluator: Sheet객체대등(확장?)이므로 필요 함수 활용가능
        // rowSpan/숨김과 상관없이 모든 row에 대해 호출됨 (0/0,1/0,2/0 ... n/0)
        if (row < this.binder.frozenRows) return true; // 열필드는 항상 표시
    ...
    NumberRowFilter.prototype.initDialog = function(col) { // ES 5이상 (arr.indexOf)
        // 동일형(column)이 content element를 공유하므로 사용시마다 초기화해야함
    ...
    NumberRowFilter.prototype.onUserSelected = function() {
        this.spanned = false;
        var controls = document.forms["f-d-n-form"].elements;
        this.opt = controls["condition"].value;
        if (this.opt === undefined) { // IE
    ...
    function onFilterButtonClick(row, col, binder) {
        ...
        var myFilter = binder.sheet.getRange(-1, col, -1, 1).tag(); // 형별 custom필터
