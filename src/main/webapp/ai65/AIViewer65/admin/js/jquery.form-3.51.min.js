...
function c(c){function d(a){var b=null;try{a.contentWindow&&(b=a.contentWindow.document)}catch(c){r("cannot get iframe.contentWindow document: "+c)}if(b)return b;try{b=a.contentDocument?a.contentDocument:a.document}catch(c){r("cannot get iframe.contentDocument: "+c),b=a.document}return b}function f(){function a(){try{var b=d(w).readyState;r("state = "+b);b&&"uninitialized"==b.toLowerCase()&&setTimeout(a,50)}catch(c){r("Server abort: ",c," (",c.name,")"),h(2),v&&clearTimeout(v),v=void 0}}var c=n.attr2("target"),
...
if(m||e.textarea){var n=q.getElementsByTagName("textarea")[0];if(n)k.responseText=n.value,k.status=Number(n.getAttribute("status"))||k.status,k.statusText=n.getAttribute("statusText")||k.statusText;else if(m){var p=q.getElementsByTagName("pre")[0],t=q.getElementsByTagName("body")[0];p?k.responseText=p.textContent?p.textContent:p.innerText:t&&(k.responseText=t.textContent?t.textContent:t.innerText)}}else"xml"==l&&!k.responseXML&&k.responseText&&(k.responseXML=E(k.responseText));try{A=G(k,l,e)}catch(D){a=
