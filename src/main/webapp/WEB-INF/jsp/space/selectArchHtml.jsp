...
<script>
	...
	//TOE 신규 개발
	var listToe = new Array();
	var listToeVal = new Array();
	...
	<c:forEach items="${listArch}" var="item">
		<c:if test="${item.energy_type eq '전기'}">
			<script>
				...
				var energy_use = '${item.energy_use_sum}';
				var toe = parseFloat(energy_use);
				var toe_ele = Math.round(toe * 0.086);
				
				listToe.push("${item.energy_type}");
				listToeVal.push(toe_ele);
	...
	<c:forEach items="${listArch}" var="item">
		<c:if test="${item.energy_type eq '가스'}">
			<script>
				...
				var energy_use = '${item.energy_use_sum}';
				var toe = parseFloat(energy_use);
				var toe_gas = Math.round(toe * 1.029);
				
				listToe.push("${item.energy_type}");
				listToeVal.push(toe_gas);
	...
	<c:forEach items="${listArch}" var="item">
		<c:if test="${item.energy_type eq '지역난방'}">
			<script>
				...
				var energy_use = '${item.energy_use_sum}';
				var toe = parseFloat(energy_use);
				var toe_heat = Math.round(toe * 0.1);
				
				listToe.push("${item.energy_type}");
				listToeVal.push(toe_heat);
...
<script>
	//Line
	console.log("listEnergy_Use : "+listEnergy_Use);
	console.log("listTCo2eq : "+listTCo2eq);
	console.log("listEnergy_UseVal : " +listEnergy_UseVal);
	console.log("listTco2eqVal : " + listTco2eqVal);
	...
	var energyData = {
		labels: listEnergy_Use, listTCo2eq, listToe,
		datasets: [
			{
					...
					<c:forEach items="${listArch}" var="item">
						<c:if test="${fn:contains(item.energy_type, '전기')}">
							<tr>
								...
								<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${item.energy_use_sum * 0.086}" /></td>
							</tr>
						</c:if>
					</c:forEach>
					...
					<c:forEach items="${listArch}" var="item">
						<c:if test="${fn:contains(item.energy_type, '가스')}">
							<tr>
								...
								<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${item.energy_use_sum * 1.029}" /></td>
							</tr>
						</c:if>
					</c:forEach>
					...
					<c:forEach items="${listArch}" var="item">
						<c:if test="${fn:contains(item.energy_type, '지역난방')}">
							<tr>
								...
								<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${item.energy_use_sum * 0.1}" /></td>
							</tr>
						</c:if>
					</c:forEach>
				</c:if>
