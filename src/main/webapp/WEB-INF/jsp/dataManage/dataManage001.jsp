...
<script>
	...
	var globalCloseMonth = new Object();
	function search() {
		globalCloseMonth = new Object();
		...
		callAjax("${pageContext.request.contextPath}/dataManage/dataManage001_search.do",true,'json',param, function(data) {
			...
			for (var i = 1; i <= 12; i++) {
				globalCloseMonth["CLOSE_MON_"+i] = data[0]["CLOSE_YN_"+i];
			}
			...
			for (key in data) {
				...
				tr += '<td>  <input type="text" class="comma" name="GHG_VAL_YEAR_'+data[key]["INV_ENG_POINT_ID"]+'" id="GHG_VAL_YEAR_'+data[key]["INV_ENG_POINT_ID"]+'" value="'+num_comma(data[key]["GHG_VAL_YEAR"])+'" onkeypress="return isNumberKey(event)" onkeyup="return delHangle(event)" readonly> </td>';
				tr += '</tr>';
			}

			if (isNoData) {
				tr += '<tr>';
				tr += '<td k="20">조회된 데이터가 없습니다.</td>';
				tr += '</tr>';
			}
			$("#listTable_body").append(tr);
			
	 		$('#listTable_body tr:visible').each(function(row) {
				$('#listTable_body').colspan(row, 6);
			}); 
			
			$('#listTable_body tr:visible').each(function(cols) {
				$('#listTable_body').rowspan(cols,7,'');
			});
	...
	function saveMonthData(){
		var data = new Object();
		var isStop = false;
		$('#monthTable_body tr:visible').each(function(row) {
			
			var len = $(this).find("input").length;
			var tempValueArray= new Array();
			for(var i =0;i<len;i++){
				/* if( $(this).find("input").eq(i).attr("class").indexOf("req") > -1  &&  ($(this).find("input").eq(i).val() == "" || $(this).find("input").eq(i).val() == "0")){
					alert('값은 0 이상 및 필수 입력입니다.');
					isStop = true;
		...
		if(!isStop){
			var param = {data : JSON.stringify(data), POINT_CODE : $("#title").html()};
		 	  callAjax("${pageContext.request.contextPath}/dataManage/dataManage001_saveMonth.do",true,'text',param, function(data) {
				alert(data);
			}) 
		}
	}
	...
	function sampleDown(){
		location.href= "/sample/dataManage001_month.xlsx";
	}
...
<body>
		...
		<div class="main-content">
			<div class="content-heading">
				<div class="heading-left">
					<h1 class="page-title">활동자료 등록(직접배출)</h1>
					<p class="page-subtitle">서울특별시 인벤토리 CRF 배출별 자료 등록</p>
				</div>
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">
							<a href="/dataManage/dataManage001.do">활동자료 등록(직접배출)</a>
						</li>
					</ol>
				</nav>
			</div>
							...
							<form name="searchFrm" id="searchFrm">
								<div class="project-heading">
									<div class="row">
										...
										<div class="checkDiv">
											<span class="type-name" style="width:90px;">연간자료연도</span> 
											<select class="" id="annualYear" name="annualYear">
												<option value="" selected>전체</option>
												<c:forEach var="item" items="${yearList}">
													<option value="${item.YYYY}">${item.YYYY}</option>
												</c:forEach>
										...
										<div class="checkDiv">
											...
											<!-- <button type="button" class="btn btn-primary" aria-expanded="false" id="excelDownload" onclick="openModal('excelUploadModal');">확정일괄등록</button> -->
											<button type="button" class="btn btn-primary" aria-expanded="false" id="excelDownload" onclick="excelExport();">엑셀다운</button>
										</div>
									</div>
								</div>
							</form>
							...
							<div class="row">
								<div class="col-lg-12" style="max-height: 600; min-height: 600;overflow-y: auto">
									<table id="listTable" class="table table-striped table-project-tasks">
										<tr>
											<td  style="word-break: normal;">1</td>
											<td  style="word-break: normal;">1A</td>
											<td  style="word-break: normal;">1A1</td>
											<td  style="word-break: normal;">1A1a</td>
											<td  style="word-break: normal;">1A1ai</td>
											<td  style="word-break: normal;">1A1ai1</td>
	...
	<!-- END MAIN -->
	<div class="modal fade" id="excelUploadModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="z-index: 99999">
				...
				<div class="modal-body">
					<form name="excelUploadFrm" id="excelUploadFrm" class="form-inline" method="post" enctype="multipart/form-data">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input id="excelfile1" type="file" name="file" class="">
							</div>
						</div>
					</form>
				</div>
	...
	<div class="modal fade" id="excelUploadModalMonth" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="z-index: 99999">
		<div class="modal-dialog excelUploadModal" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">엑셀 업로드</h5>
				...
				<div class="modal-body">
					<form name="excelUploadFrm" id="excelUploadFrm" class="form-inline" method="post" enctype="multipart/form-data">
						<div class="form-group">
							<label class="col-sm-4 control-label">엑셀 파일 업로드</label>
							<div class="col-sm-8">
								<input id="excelfile2" type="file" name="file" class="">
							</div>
						</div>
					</form>
				</div>
				<div class="modal-footer">
					<button class="btn btn-secondary" type="button"data-dismiss="modal">취소</button>
					<button onclick="javascript: uploadExcelItemMonth()" type="button" class="btn btn-success pull-right">업로드</button>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="modal-dialog monthDataManager" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button class="btn btn-secondary" type="button" onclick="sampleDown()">엑셀샘플</button>
					<button onclick="openModal('excelUploadModalMonth');" type="button" class="btn btn-success pull-right">일괄등록</button>
					<button onclick="javascript: saveMonthData()" type="button" class="btn btn-success pull-right">등록</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script>
		...
		var color = Chart.helpers.color;
			$(document).ready(	function() {
				$("#excelfile1").on("change", function(e) {
		...
		function uploadExcelItem(){
			...
			originData = originData.replace(/해당년도/gi,"YEAR_DATA");
			
			var data = JSON.parse(originData);
			for(key in data){
				$("#GHG_VAL_YEAR_"+data[key]["INV_ENG_GHG_YEAR_ID"]).val(data[key]["YEAR_DATA"]);	
			}
			$("#excelUploadModal").modal('hide');
			$("#excelData").val("");
		}
		
		function uploadExcelItemMonth(){
			var originData = $("#excelData").val();
			originData = originData.replace(/에너지원 CODE/gi,"S_CD")
								   .replace(/에너지원/gi,"S_DESC")
								   .replace(/단위코드/gi,"ENG_UNIT")
								   .replace(/단위/gi,"ENG_UNIT_NM")
								   .replace(/필수여부/gi,"VITAL_YN")
								   .replace(/12월/gi,"N_ENG_12")
								   .replace(/11월/gi,"N_ENG_11")
								   .replace(/10월/gi,"N_ENG_10")
								   .replace(/1월/gi,"N_ENG_1")
								   .replace(/2월/gi,"N_ENG_2")
								   .replace(/3월/gi,"N_ENG_3")
								   .replace(/4월/gi,"N_ENG_4")
								   .replace(/5월/gi,"N_ENG_5")
								   .replace(/6월/gi,"N_ENG_6")
								   .replace(/7월/gi,"N_ENG_7")
								   .replace(/8월/gi,"N_ENG_8")
								   .replace(/9월/gi,"N_ENG_9")
			...
			for(key in data){
				//tr += "<tr>";
				tr += "<tr id='"+data[key]["S_CD"]+"_"+$("#year").val()+"_"+data[key]["ENG_UNIT"]+"_"+ data[key]["VITAL_YN"]+"'>";
				tr += "<td>"+ data[key]["S_DESC"] +"</td>";
				tr += "<td>"+ data[key]["VITAL_YN"] +"</td>";
				tr += "<td>"+ data[key]["ENG_UNIT_NM"] +"</td>";
				
				var sum = 0;
				var isVital = '';
				for(var i=1;i<=12;i++){
					var id = data[key]["S_CD"] + "_"  + $("#year").val()+ "_" + "N_ENG_"+ i;
					var val = data[key]["N_ENG_"+i] == undefined ? 0 : Number(data[key]["N_ENG_"+i].replace(/,/gi,""));
					sum += val;
					...
					var isClose = globalCloseMonth["CLOSE_MON_"+i] == "Y" ? "readonly disabled" : "";
					tr += "<td ><input type='text' name='"+id+"' id='"+id+"' value='"+num_comma(val)+"' style='width:80px' onkeypress='return isNumberKey(event)' onkeyup='return delHangle(event)' class='comma "+isVital+"' "+isClose+"> </td>";
				}
				...
				$("#excelUploadModalMonth").modal('hide');
				$("#excelData").val("");
				$("#excelfile2").val("");
		}
		
		function excelExport() {
			ExcellentExport.excel(this, 'listTable', "활동자료 등록", "uploadExcel", "name");
		}
		 ...
		 */
		 ...
		 function execute(){
			 ...
			 if($("#annualYear").val() == ""){
				 alert("연간자료연도를 선택해 주세요.");
				 $("#annualYear").focus();
				 return;
			 }
			 var param = $("#searchFrm").serialize();
			 callAjax("${pageContext.request.contextPath}/dataManage/dataManage001_execute.do",true,'json',param, function(data) {
				 if(data == "S"){
					 alert("성공적으로 처리되었습니다.");
				 }
			 });
