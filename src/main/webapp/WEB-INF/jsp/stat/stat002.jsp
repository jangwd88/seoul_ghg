...
<script>
	...
	function search() {
		...
		callAjax(
				"${pageContext.request.contextPath}/stat/stat002_search.do",
				true,
				'json',
				param,
				function(data) {
					...
					var tr = "";
					tr += '<tr id="tHeader1">';
					tr += '<td rowspan="3" style="width: 200px; padding-left: 0px; word-break: normal;">자치구</td>';
					tr += '<td rowspan="3" style="width: 200px; padding-left: 0px; word-break: normal;">법정동</td>';
					var colspanCnt = 0;
					...
					for (key in data) {
						tr += '<tr class="data">'
						tr += '<td>' + data[key]["SIGUNGU_NM"] + '</td>'
						tr += '<td>' + data[key]["BJDONG_NM"] + '</td>'
						for (i = startVar; i <= endVar; i++) {
							tr += '<td>'
								+ num_comma((parseInt(data[key]["E_TCO2EQ_" + i]) + parseInt(data[key]["G_TCO2EQ_" + i]) + parseInt(data[key]["H_TCO2EQ_" + i])).toFixed(0)) + '</td>';
							if ($("input[name='energy0']").prop("checked") == true) {
								tr += '<td>' + num_comma(data[key]["E_TOE_" + i].toFixed(0)) + '</td>';
								tr += '<td>' + num_comma(data[key]["E_TCO2EQ_" + i].toFixed(0)) + '</td>';
							}
							if ($("input[name='energy1']").prop("checked") == true) {
								tr += '<td>' + num_comma(data[key]["G_TOE_" + i].toFixed(0)) + '</td>';
								tr += '<td>' + num_comma(data[key]["G_TCO2EQ_" + i].toFixed(0)) + '</td>';
							}
							if ($("input[name='energy2']").prop("checked") == true) {
								tr += '<td>' + num_comma(data[key]["H_TOE_" + i].toFixed(0)) + '</td>';
								tr += '<td>' + num_comma(data[key]["H_TCO2EQ_" + i].toFixed(0)) + '</td>';
							}
						}
						tr += '</tr>';
					}
					if (tr == "") {
						tr += '<tr class="data">'
						tr += '<td colspan="9">조회 결과가 없습니다.</td>'
						tr += '</tr>'
					}
					$("#listTable").append(tr);
					$('table tbody tr:visible').each(function(cols) {
						$('#listTable').colspan(cols, 1);
					})

					$('table tbody tr:visible').each(function(cols) {
						$('#listTable').rowspan(cols, 1);
					})
	...
	function createChartData(data, type, startVar, endVar) {
		...
		for (key in data) {
			tempArr.push(data[key]["BJDONG_NM"]);
		}
		...
		for (key in uniArr) {
			var jsonData = getJsonFilter(data, "BJDONG_NM", uniArr[key]);
		...
		<div class="main-content">
			<div class="content-heading">
				<div class="heading-left">
					<h1 class="page-title">지역별 사용 통계</h1>
					<p class="page-subtitle">서울특별시 건축물 지역별 에너지 사용량 통계</p>
				</div>
				<nav aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">
							<a href="/stat/stat002.do">지역별 사용통계</a>
						</li>
					</ol>
				</nav>
			</div>
						...
						<div class="card">
							<form name="searchFrm" id="searchFrm">
								<div class="project-heading">
									...
									<div class="row">
										<div class="checkDiv">
											<span class="type-name">자치구</span>
											<select class="" id="sigungu" name="sigungu">
												<option value="" selected>선택</option>
												<c:forEach var="item" items="${SIGUNGU}">
													<option value="${item.EXT_VALUE}">${item.VALUE}</option>
												</c:forEach>
											</select>
										</div>
										<div class="checkDiv">
											<span class="type-name">법정동</span>
											<input type="hidden" name="bjdong" id="bjdong">
											<button type="button" class="btn btn-primary" aria-expanded="false" id="bjdongBtn" onclick="openModal('modalPop_001')">선택</button>

											<%--  <select class="" id="bjdong" name="bjdong">
												<option value="" selected>전체</option>
												<c:forEach var="item" items="${BJDONG}">
							...
							<div class="row">
								<div class="col-lg-12" style="margin-left: 10px;">
									<div class="checkDiv">
										<span class="type-name">차트</span> <select class="" id="TYPE">
							...
							<div class="row">
								<div class="col-lg-12" style="max-height: 300; overflow-y: scroll; padding-right: 30px; padding-left: 30px">
									<table id="listTable" class="table table-striped table-project-tasks">
										<tr id="tHeader1">
											<td rowspan="3" style="width: 200px; padding-left: 0px; word-break: normal;">자치구</td>
											<td rowspan="3" style="width: 200px; padding-left: 0px; word-break: normal;">법정동</td>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">법정동</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body" style="overflow: auto; height: 400px;">
					<table class="table table-striped table-project-tasks" id="bjDongTable">
						...
						<%-- <c:forEach var="item" items="${BJDONG}" varStatus="status">
							<tr onclick="checkLine('BJDONG_${status.index}')">
								<td><input type="checkbox" value="${item.CODE}" id="BJDONG_${status.index}" class="tempBjdong"></td>
								<td>${item.VALUE}</td>
							</tr>
						</c:forEach> --%>
					</table>
	...
	<script>
		...
		$(document)
				.ready(
						function() {
							...
							$("#TYPE").on(
									...
									function() {
										if (globalSearchData != "") {
											...
											createChartData(globalSearchData,$("#TYPE").val(),startVar,endVar);
										}
									})
							...
							$("#sigungu")
									.on(
											...
											function() {
												...
												callAjax(
														"${pageContext.request.contextPath}/stat/stat002_getBjdong.do",
														true,
														'json',
														param,
														function(data) {
															...
															for (key in data) {
																...
																tr += "<tr onclick=\""
																		...
																		+ "')"
																		+ "\"  class='bjData'>"
																tr += '<td><input type="checkbox" value="'+data[key]["CODE"]+'" id="BJDONG_'+key+'" class="tempBjdong"></td>'
																tr += '<td>'
																		+ data[key]["VALUE"]
																		+ '</td>'
																tr += '</tr>'
							...
							$('table tbody tr:visible').each(function(cols) {
								$('#listTable').colspan(cols, 2);
							})
							$('table tbody tr:visible').each(function(cols) {
								$('#listTable').rowspan(cols, 2);
							})
							...
							$("#end_year")
									...
									.change(
											function() {
												...
												var startDate = Number(String($(
														"#start_year").val())
														+ String($(
																"#start_month")
																.val()));
												var endDate = Number(String($(
														"#end_year").val())
														+ String($("#end_month")
																.val()));
		...
		function checkLine(id) {
			...
			if (type.indexOf("checkbox") > -1) {
				...
				if (checkpoint) {
					$(".tempBjdong").prop("checked", checked);
				}
				if (!checked) {
		...
		function confirmModal(id) {
			if (id == "modalPop_001") {
				var len = $('.tempBjdong:checked').length;
				var name = "";
				for (var i = 0; i < len; i++) {
					if (i == 3) {
						break;
					}
					name += $('.tempBjdong:checked').eq(i).parent().next()
							.html()
							+ ",";
				}
				...
				if (len > 3) {
					$("#bjdongBtn").html(name + "외 " + (len - 3) + " 건");
				} else {
					$("#bjdongBtn").html(name);
				}
				var value = "";
				for (var i = 0; i < $('.tempBjdong:checked').length; i++) {
					value += $('.tempBjdong:checked').eq(i).val() + ",";
				}
