...
var ExcellentExport = (function() {
    ...
    var ee = {
        /** @export */
        excel : function(anchor, table, sheetname,filename,isDelete) {
            table = get(table);

            //Auto Merge 기능 사용 시 td에 display css를 부여 함
            //해당 엑셀 기능은 display 여부에 상관없이 가져오기 때문에 엑셀 파일이 깨짐 
            //바로 아래 기능을 이용해 display none된 데이터는 삭제시킴
            if(isDelete != "N"){
            	$("td:hidden").remove();
            }
            ...
            return createDownloadLink(anchor, b64, 'application/vnd.ms-excel',filename+'.xls');
        },
        /** @export */
